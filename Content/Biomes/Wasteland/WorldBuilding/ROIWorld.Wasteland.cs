    ...
    partial class ROIWorld : ModWorld
    {
        ...
        private void WastelandUraniumOreGen(GenerationProgress progress)
        {
            for (int i = 0; i < 50; i++)
            {
                ...
                if (Scan(x, Main.maxTilesY - 175, out yGen, (tile => { return true; })))
                {
                    ...
                    {
                        new Actions.PlaceTile((ushort)ModContent.TileType<UraniumOre>(), 0)
                    }));
                }
            }
        }
        ...
        private void GenerateCavern(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX * 0.15; i++)
            {
                ...
                GeneralWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double)WorldGen.genRand.Next(2, 5), WorldGen.genRand.Next(20, 50), ModContent.TileType<WastelandDirt>());
            }
        }
        ...
        internal void TerrainTop(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                for (int j = Main.maxTilesY - 200; j < Main.maxTilesY - 195; j++)
                {
                    if (WorldGen.genRand.Next(20) == 0)
                    {
                        WorldGen.TileRunner(i, j, (double)WorldGen.genRand.Next(4, 6), 50, ModContent.TileType<WastelandRock>(), true);
                    }
                }
            }
