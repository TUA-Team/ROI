...
namespace ROI.Content.Projectiles
{
    ...
    public class ProjectileAIStyle : GlobalProjectile
    {
        ...
        public static void DeathRayAIStyle(Projectile projectile)
        {
            ...
            if (CenteringLaserProjectileOnAPoint(projectile, ref centerPoint))
                return;

            if (projectile.velocity.HasNaNs() || projectile.velocity == Vector2.Zero)
                projectile.velocity = -Vector2.UnitY;

            if (LaserAIUpdate(projectile))
                return;
        ...
        private static void LastPrismDust(Projectile projectile)
        {
            ...
            if (Main.rand.Next(5) == 0)
            {
                ...
                Dust secondaryDust = Dust.NewDustDirect(dustPosition + value34 - Vector2.One * 4f, 8, 8, 31, 0f, 0f, 100, default, 1.5f);
            ...
            float drippleColorModifier = 0.1f * (float)Math.Sin(Main._drawInterfaceGameTime.TotalGameTime.TotalSeconds % 3600.0 * 20f);
            ...
            if (Main.netMode != 2)
                ((WaterShaderData)Filters.Scene["WaterDistortion"].GetShader()).QueueRipple(projectile.position + new Vector2(size.X * 0.5f, 0f).RotatedBy(prjectileVelocityToRotation), new Color(0.5f, 0.1f * Math.Sign(drippleColorModifier) + 0.5f, 0f, 1f) * Math.Abs(drippleColorModifier), size, RippleShape.Square, prjectileVelocityToRotation);
