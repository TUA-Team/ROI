...
namespace ROI.Content.Subworlds.Wasteland.WorldBuilding
{
    ...
    internal static class WastelandWorldGeneration
    {
        ...
        internal static Mod mod => ContentInstance<ROIMod>.Instance;
        ...
        {
            //Bottom
            BaseTerrain(new float[] { 0.0077f, 0.0011f, 0.022f, 0.04f }, new float[] { 0.7f, 0.05f, 0.02f, 0.01f }, 450);
            //Top
            BaseTerrain(new float[] { 0.07f, 0.04f, 0.03f }, new float[] { 0.7f, 0.04f, 0.06f }, 200, true);
        ...
        private static void BaseTerrain(float[] freq, float[] limit, int startingHeight, bool top = false)
        {
            int[][] displacements = new int[freq.Length][];
            for (int i = 0; i < freq.Length; i++)
            {
                displacements[i] = GetPerlinDisplacements(Main.maxTilesX, freq[i], startingHeight, limit[i], WorldGen._lastSeed);
            }
            ...
            if (!top)
            {
                for (int i = 0; i < Main.maxTilesX; i++)
                {
                    ...
                    GeneralWorldHelper.Fill(i, totalDisplacements[i], 1, dirtDepth, (ushort)mod.TileType("Wasteland_Dirt"));
                    GeneralWorldHelper.Fill(i, totalDisplacements[i] + dirtDepth, 1, 600, (ushort)mod.TileType("Wasteland_Rock"));
            ...
            else
            {
                for (int i = 0; i < Main.maxTilesX; i++)
                {
                    ...
                    int dirtDepth = WorldGen.genRand.Next(10, 15);
                    GeneralWorldHelper.FillInvertYAxis(i, totalDisplacements[i], 1, 400, (ushort)mod.TileType("Wasteland_Rock"));
                }
            }
        ...
        {
            ushort dirtType = (ushort)ModContent.TileType<WastelandDirt>();
        ...
        private static void GenerateCavern(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX * 0.15; i++)
            {
                ...
                GeneralWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double)WorldGen.genRand.Next(15, 20), WorldGen.genRand.Next(30, 40), type);
                GeneralWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double)WorldGen.genRand.Next(8, 10), WorldGen.genRand.Next(20, 50), mod.TileType("Wasteland_Dirt"));
            }
