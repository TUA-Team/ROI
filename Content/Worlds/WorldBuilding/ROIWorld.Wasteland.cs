...
namespace ROI.Content.Worlds
{
    ...
    {
        private static Dictionary<int, int> SurfaceLevel;
        private static int HighestLevel = 300;
        ...
        private void WastelandUraniumOreGen(GenerationProgress progress)
        {
            for (int i = 0; i < 50; i++)
            {
                ...
                if (Scan(x, Main.maxTilesY - 175, out yGen, (tile => { return true; })))
                {
                    ...
                    {
                        new Actions.PlaceTile((ushort)ModContent.TileType<Uranium_Ore>(), 0)
                    }));
                }
            }
        }
        ...
        private void BaseTerrain(float[] freq, float[] limit, bool top = false)
        {
            ...
            if (!top)
            {
                for (int i = 0; i < Main.maxTilesX; i++)
                {
                    ...
                    Fill(i, totalDisplacements[i], 1, dirtDepth, (ushort)ModContent.TileType<WastelandDirt>());
                    Fill(i, totalDisplacements[i] + dirtDepth, 1, 200, (ushort)ModContent.TileType<WastelandRock>());
            ...
            for (int num11 = 0; num11 < (int)(Main.maxTilesX * Main.maxTilesY * 0.0008); num11++)
            {
                if (WorldGen.genRand.Next(20) == 0)
                    WorldGen.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX),
                        WorldGen.genRand.Next(Main.maxTilesY - 140, Main.maxTilesY),
                        WorldGen.genRand.Next(2, 7),
                        WorldGen.genRand.Next(3, 7),
                        ModContent.TileType<WastelandOre>(),
                        false,
                        0f,
                        0f,
                        false,
        ...
        private void GeneratingRuins(GenerationProgress progress)
        {
            ...
            for (int i = (int)(Main.maxTilesX * 0.30); i < Main.maxTilesX * 0.70; i++)
            {
                if (nextHouseCooldown <= 0 && WorldGen.genRand.Next(5) == 0)
                {
                    ...
                    {
                        if (Main.tile[i, j].type == ModContent.TileType<Tiles.Wasteland.WastelandDirt>())
                        {
        ...
        {
            ushort dirtType = (ushort)ModContent.TileType<Tiles.Wasteland.WastelandDirt>();
        ...
        private void GenerateCavern(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX * 0.15; i++)
            {
                ...
                GeneralWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double)WorldGen.genRand.Next(5, 8), WorldGen.genRand.Next(50, 70), type);
                GeneralWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double)WorldGen.genRand.Next(2, 5), WorldGen.genRand.Next(20, 50), ModContent.TileType<Tiles.Wasteland.WastelandDirt>());
            }
        }
        ...
        internal void TerrainTop(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                for (int j = Main.maxTilesY - 200; j < Main.maxTilesY - 195; j++)
                {
                    if (WorldGen.genRand.Next(20) == 0)
                    {
                        WorldGen.TileRunner(i, j, (double)WorldGen.genRand.Next(4, 6), 50, ModContent.TileType<Tiles.Wasteland.WastelandRock>(), true);
                    }
                }
            }
            ...
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                for (int j = Main.maxTilesY - 200; j < Main.maxTilesY; j++)
                {
                    if (Main.tile[i, j].type != ModContent.TileType<Tiles.Wasteland.WastelandRock>())
                    {
                        Main.tile[i, j].active(false);
                        Main.tile[i, j].liquid = 0;
                        Main.tile[i, j].wall = 0;
        ...
        internal void TerrainBottom(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                ...
                for (int j = wastelandHeight; j < wastelandRock; j++)
                {
                    ...
                    Main.tile[i, j].slope(0);
                    Main.tile[i, j].type = (ushort)ModContent.TileType<Tiles.Wasteland.WastelandDirt>();
                    Main.tile[i, j].wall = (ushort)ModContent.WallType<Walls.Wasteland.WastelandDirtWall>();
                }
                ...
                for (int j = wastelandRock; j < Main.maxTilesY; j++)
                {
                    Main.tile[i, j].active(true);
                    Main.tile[i, j].slope(0);
                    Main.tile[i, j].type = (ushort)ModContent.TileType<Tiles.Wasteland.WastelandRock>();
                    Main.tile[i, j].wall = (ushort)ModContent.WallType<Walls.Wasteland.WastelandRockWall>();
                }
            }
        }
        ...
        private static void WastelandForgeGen(GenerationProgress progress)
        {
            ...
            for (int num258 = 0; num258 < Main.maxTilesX / 200; num258++)
            {
                ...
                while (!flag17)
                {
                    int num260 = WorldGen.genRand.Next(1, Main.maxTilesX);
                    int num261 = WorldGen.genRand.Next(Main.maxTilesY - 250, Main.maxTilesY - 5);
                    try
                    {
                        if (Main.tile[num260, num261].wall == ModContent.WallType<WastestoneBrickWall>())
                        {
                            ...
                            num261--;
                            WorldGen.PlaceTile(num260, num261, ModContent.TileType<WastelandForge>());
                    ...
                    {
                        ROIMod.Instance.Logger.Error("Error generating wasteland", e);
                    }
                }
            }
        }
