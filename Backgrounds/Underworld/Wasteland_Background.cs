...
{
    class Wasteland_Background
    {
        private static Texture2D[] texture = new Texture2D[5];
        public static void Load()
        {
            for (int i = 0; i < texture.Length; i++)
            {
                ROIMod.instance.GetTexture("Backgrounds/Underworld/Wasteland_" + i);
            }
        ...
        public static void DrawUnderworldBackground(On.Terraria.Main.orig_DrawUnderworldBackground orig, Main instance, bool flat)
        {
            ...
            for (int i = 4; i >= 0; i--)
            {
                ...
                if (flat)
                {
                    zero.Y += (float)(texture[0].Height >> 1) * 1.3f - vector.Y;
                }
                zero.Y -= num;
