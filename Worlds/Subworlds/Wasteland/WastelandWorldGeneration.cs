...
namespace ROI.Worlds
{
...

    {
        ...
        private static void BaseTerrain(float[] freq, float[] limit, int[] octave, int startingHeight, bool top = false)
        {
            ...
            if (!top)
            {
                for (int i = 0; i < Main.maxTilesX; i++)
                {
                    ...
                    ROIWorldHelper.Fill(i, totalDisplacements[i] + dirtDepth, 1, 900, (ushort)mod.TileType("Wasteland_Rock"));
        ...
        private static void GenerateWasteLake(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                ...
                })))
                {
                    ...
                    {
                        Wasteland_Lake.Generate(x, yGen, 40, 55, 8f, 3, 3, false);
                        bigLake++;
                        continue;
                    }

                    Wasteland_Lake.Generate(x, yGen, 20, 24, 5f, 3, 3, true);
                ...
                if (nextHouseCooldown <= 0 && WorldGen.genRand.Next(5) == 0)
                {
                     ...
                     }))
                    {
                        if (Main.tile[i, j].type == mod.TileType("Wasteland_Dirt"))
                        {
                            ...
                            Wasteland_Ruins.PlaceHouse(i, j);
                        }
                    }
                }
        ...
        private static void GeneratePerlinCavern(GenerationProgress progress)
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (WorldGen.genRand.Next(5) == 0)
                {
                    ...
                    ROIWorldHelper.TileRunner((int)coordinate.X, (int)coordinate.Y, strenght - 5, WorldGen.genRand.Next(15, 20), -5, 0, false, WorldGen.genRand.NextFloat(-3f, 3f), WorldGen.genRand.NextFloat(-3f, 3f));
                    ROIWorldHelper.TileRunner((int)coordinate.X, (int)coordinate.Y, strenght + 4, WorldGen.genRand.Next(15, 20), ModContent.TileType<Wasteland_Waste>(), 0, false, WorldGen.genRand.NextFloat(-3f, 3f), WorldGen.genRand.NextFloat(-3f, 3f));
                }
                else
                {
        ...
        private static void GenerateMossCavern(GenerationProgress progress)
        {
            ...
            Wasteland_Lost_Cave lostCave = new Wasteland_Lost_Cave(new Rectangle(50, 500, 200, 200));
