...
namespace ROI.Worlds
{
    ...
    internal static Mod mod => ROIMod.instance;
    ...
    private static void BaseTerrain(float[] freq, float[] limit, int startingHeight, bool top = false)
    {
        ...
        else
        {
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                ...
                int dirtDepth = WorldGen.genRand.Next(10, 15);
                ROIWorldHelper.FillInvertYAxis(i, totalDisplacements[i], 1, 400, (ushort) mod.TileType("Wasteland_Rock"));
            }
        }
    ...
    private static void WastelandOreGeneration(GenerationProgress progress)
    {
        ...
        {
            if (WorldGen.genRand.Next(20) == 0)
                WorldGen.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(400, Main.maxTilesY), (double) WorldGen.genRand.Next(2, 7), WorldGen.genRand.Next(3, 7), ModContent.TileType<Wasteland_Ore>(), false, 0f, 0f, false, true);
        }
    }
    ...
    private static void GenerateCavern(GenerationProgress progress)
    {
        ...
        for (int i = 0; i < Main.maxTilesX * 0.15; i++)
        {
            ...
            ROIWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double) WorldGen.genRand.Next(15, 20), WorldGen.genRand.Next(30, 40), type);
            ROIWorldHelper.TileRunner(WorldGen.genRand.Next(0, Main.maxTilesX), WorldGen.genRand.Next(MAX_WASTELAND_HEIGHT, Main.maxTilesY), (double) WorldGen.genRand.Next(8, 10), WorldGen.genRand.Next(20, 50), mod.TileType("Wasteland_Dirt"));
        }
